...
resolution.missing_fields is a list of plain field NAMES such as ["parameters.time_range"], never objects — objects with a source belong in resolution.inherited_fields. Leave missing_fields empty and clarification_question empty whenever needs_clarification is false. If information required to choose a safe plan is missing, set needs_clarification=true, use task=unclear and operation=clarify, list the missing field names, and write one concise Thai clarification_question.
