...
func localAnalyticalGuardrailAnswer(question string, snapshot AISnapshot) (string, bool) {
	...
	if !readiness.HasSales {
		return fmt.Sprintf("ตอนนี้ยังไม่มีข้อมูลยอดขายในช่วง %s จึงยังแนะนำการปรับราคา ถอดเมนู หรือสั่งซื้อวัตถุดิบไม่ได้ครับ กรุณาบันทึกการขายและตรวจว่าสูตรวัตถุดิบพร้อมก่อน แล้วผมจะช่วยวิเคราะห์ต่อได้", analysisWindowLabel()), true
	}
	return fmt.Sprintf(
