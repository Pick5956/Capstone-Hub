...
const (
	...
	// Creating a menu item: name, price, one category. Recipe, image and
	// options stay on the menu screen — the assistant makes the row, the owner
	// fills it in.
	AIActionTypeCreateMenuItem = "create_menu_item"
...
type AIActionPlanItem struct {
	...
	ActionType string `json:"action_type" gorm:"size:48;not null;check:ai_action_plan_items_allowed_type,action_type IN ('adjust_ingredient_stock','set_ingredient_min_stock','set_ingredient_cost','create_ingredient','set_menu_availability','create_expense','set_menu_price','create_menu_item')"`
