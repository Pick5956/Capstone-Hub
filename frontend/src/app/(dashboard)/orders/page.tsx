...
export default function OrdersPage() {
  ...
  }, [canView, debouncedQuery, ordersLoadError, ordersLoadMoreError, paymentFilter, statusFilter]);
  ...
  const statusFilters: { value: StatusFilter; label: string; count: number }[] = [
    ...
    { value: "closed", label: copy.closed, count: summary.closed },
    { value: "open", label: copy.open, count: summary.statuses.open },
    { value: "ready", label: copy.ready, count: summary.statuses.ready },
  ...
  return (
    <OperationalPageShell
      ...
      edgeToEdge
    >
      <section className="flex min-h-0 flex-1 flex-col border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950">
        ...
        <div className="grid shrink-0 gap-3 border-b border-gray-200 p-3 dark:border-gray-800 lg:grid-cols-[minmax(220px,1fr)_auto] lg:items-center">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div role="group" aria-label={copy.payment} className="flex flex-1 gap-2 sm:flex-none">
              {(["all", "unpaid", "paid"] as PaymentFilter[]).map((filter) => (
                <button
                  ...
                  className={`h-10 flex-1 rounded-md px-3 text-[12px] font-semibold transition-colors sm:flex-none ${
        ...
        <div role="group" aria-label={copy.status} className="soft-scrollbar-hide flex shrink-0 gap-2 overflow-x-auto border-b border-gray-200 px-3 py-2 dark:border-gray-800">
          {statusFilters.map((item) => (
            <button
              ...
              className={`inline-flex shrink-0 items-center gap-2 rounded-md px-3 py-1.5 text-[12px] font-semibold transition-colors ${
            ...
            >
              ...
              <span className={`min-w-5 text-right font-mono text-[11px] tabular-nums ${statusFilter === item.value ? "text-orange-600 dark:text-orange-300" : "text-gray-400 dark:text-gray-500"}`}>
        ...
        >
          <div className="min-w-0 lg:min-w-[1060px]">
            <div className="sticky top-0 z-10 hidden border-b border-gray-200 bg-white px-4 py-2 text-center text-[11px] font-semibold uppercase tracking-[0.12em] text-gray-400 dark:border-gray-800 dark:bg-gray-950 lg:grid lg:grid-cols-[minmax(90px,0.8fr)_minmax(80px,0.7fr)_minmax(100px,1fr)_minmax(120px,1fr)_minmax(90px,0.8fr)_minmax(90px,0.8fr)_minmax(80px,0.7fr)_minmax(100px,0.9fr)_minmax(170px,1.4fr)] lg:items-center lg:gap-3">
            ...
            {loading ? (
              ...
              <div className="divide-y divide-gray-100 dark:divide-gray-900">
                {orders.map((order) => (
                  <div
                    key={order.ID}
                    className="grid w-full gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50 dark:hover:bg-gray-900/60 lg:grid-cols-[minmax(90px,0.8fr)_minmax(80px,0.7fr)_minmax(100px,1fr)_minmax(120px,1fr)_minmax(90px,0.8fr)_minmax(90px,0.8fr)_minmax(80px,0.7fr)_minmax(100px,0.9fr)_minmax(170px,1.4fr)] lg:items-center"
                  >
                    <div className="min-w-0 lg:w-full lg:text-center">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[12px] text-gray-500 dark:text-gray-400 lg:hidden">
                    ...
                    <span className="hidden truncate text-center text-[12px] font-semibold text-gray-700 dark:text-gray-200 lg:block">
                      {tableName(order, language)}
                    </span>
                    <span className="hidden truncate text-center text-[12px] text-gray-500 dark:text-gray-400 lg:block">
                      {zoneName(order) || "-"}
                    </span>
                    <span className="hidden whitespace-nowrap text-center text-[12px] tabular-nums text-gray-500 dark:text-gray-400 lg:block">
                      {formatTime(orderTime(order))}
                    </span>
                    <span className={`hidden w-fit justify-self-center rounded-md border px-2 py-1 text-[11px] font-semibold lg:inline-flex ${statusClass[order.status]}`}>
                      {statusLabel(order.status)}
                    </span>
                    <span className="text-[12px] font-semibold text-gray-600 dark:text-gray-300 lg:text-center">{statusLabel(order.payment_status)}</span>
                    <span className="text-[12px] text-gray-500 lg:text-center">{itemCount(order)} {copy.items}</span>
                    <span className="font-mono text-[14px] font-semibold tabular-nums text-gray-950 dark:text-white lg:text-center">
