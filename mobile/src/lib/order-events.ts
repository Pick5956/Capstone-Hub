...
export function parseOrderChangeEvent(
  message: Pick<ServerSentEvent, 'event' | 'data'>,
): OrderChangeEvent | null {
...
export function isKitchenOrderChangeEvent(event: OrderChangeEvent) {
  return kitchenEventActions.has(event.action);
}
