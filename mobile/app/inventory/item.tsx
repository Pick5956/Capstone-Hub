...
export default function InventoryItemScreen() {
  ...
  return (
    <AppScreen title={screenTitle} subtitle={readOnly ? copy('ข้อมูลและประวัติสต็อกแบบอ่านอย่างเดียว', 'Read-only ingredient data and stock history') : copy('ข้อมูลต้นทุนและระดับสต็อกจะใช้ในรายงานร้าน', 'Cost and stock levels are used in restaurant reports.')} topLevel={false}>
      ...
      {readOnly ? (
        <Surface>
          <SectionHeader title={copy('ข้อมูลวัตถุดิบ', 'Ingredient details')} detail={copy('บัญชีนี้ดูข้อมูลได้ แต่ไม่สามารถแก้ไขหรือปรับสต็อก', 'This account can view data but cannot edit the ingredient or adjust stock.')} />
        ...
        <TextField label={copy('ชื่อวัตถุดิบ', 'Ingredient name')} value={name} onChangeText={setName} />
        ...
        <ChipGroup label={copy('การจัดเก็บ', 'Storage')} value={storageType} onChange={setStorageType} options={[{ label: copy('อุณหภูมิห้อง', 'Room temperature'), value: 'room_temp' }, { label: copy('แห้ง', 'Dry'), value: 'dry' }, { label: copy('แช่เย็น', 'Chilled'), value: 'chilled' }, { label: copy('แช่แข็ง', 'Frozen'), value: 'frozen' }]} />
        <Button label={editing ? copy('บันทึกการเปลี่ยนแปลง', 'Save changes') : copy('เพิ่มวัตถุดิบ', 'Add ingredient')} onPress={save} loading={saving} />
      </Surface>}

      {editing && canManage ? (
        <Surface>
          <SectionHeader title={copy('ปรับสต็อก', 'Adjust stock')} detail={copy(`คงเหลือปัจจุบัน ${Number(stock).toLocaleString('th-TH')} ${unit}`, `Current stock ${Number(stock).toLocaleString('en-US')} ${unit}`)} />
          <ChipGroup value={adjustType} onChange={(value) => { setAdjustType(value); setAdjustError(null); }} options={[{ label: copy('รับเข้า', 'Stock in'), value: 'in' }, { label: copy('เบิกออก', 'Stock out'), value: 'out' }, { label: copy('ตั้งยอด', 'Set balance'), value: 'adjust' }]} />
          <TextField label={adjustType === 'adjust' ? copy('ยอดคงเหลือใหม่', 'New stock balance') : copy('จำนวน', 'Quantity')} value={adjustQuantity} keyboardType="decimal-pad" onChangeText={(value) => { setAdjustQuantity(value); setAdjustError(null); }} error={adjustError} />
          <TextField label={copy('หมายเหตุ', 'Note')} value={adjustNote} onChangeText={setAdjustNote} multiline />
          <Button label={copy('ยืนยันการปรับสต็อก', 'Confirm stock adjustment')} onPress={submitAdjustment} loading={saving} />
        </Surface>
      ) : null}

      {editing ? (
        <Surface>
          <SectionHeader title={copy('ประวัติสต็อก', 'Stock history')} detail={copy(`${transactions.length.toLocaleString('th-TH')} รายการล่าสุด`, `${transactions.length.toLocaleString('en-US')} recent entries`)} />
          {transactions.length ? transactions.slice(0, 20).map((item, index) => (
            <View key={item.ID}>
              ...
              <View style={{ minHeight: 58, flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
                <View style={{ flex: 1, gap: 2 }}><Text selectable style={typeScale.cardTitle}>{item.type === 'in' ? copy('รับเข้า', 'Stock in') : item.type === 'out' ? copy('เบิกออก', 'Stock out') : copy('ตั้งยอด', 'Set balance')}</Text><Text selectable style={[typeScale.caption, { color: palette.muted }]}>{item.note || copy('ไม่มีหมายเหตุ', 'No note')}</Text></View>
                <Text selectable style={typeScale.number}>{item.quantity.toLocaleString(locale)} {unit}</Text>
              </View>
            </View>
          )) : <StateMessage title={copy('ยังไม่มีประวัติสต็อก', 'No stock history yet')} detail={copy('รายการรับเข้า เบิกออก หรือตั้งยอดจะแสดงที่นี่', 'Stock receipts, issues, and balance changes will appear here.')} />}
        </Surface>
      ) : null}

      {editing && canManage ? (
        <Surface style={{ borderColor: confirmDelete ? palette.danger : palette.border }}>
          <SectionHeader title={copy('ลบวัตถุดิบ', 'Delete ingredient')} detail={confirmDelete ? copy('แตะยืนยันอีกครั้ง ข้อมูลสต็อกของรายการนี้จะถูกลบ', 'Confirm again to delete this ingredient and its stock data.') : copy('ใช้เมื่อรายการนี้ไม่ควรอยู่ในคลังอีกต่อไป', 'Use this only when the item should no longer exist in inventory.')} />
          <View style={{ flexDirection: 'row', gap: spacing.sm }}>
            {confirmDelete ? <Button variant="secondary" label={copy('ยกเลิก', 'Cancel')} onPress={() => setConfirmDelete(false)} style={{ flex: 1 }} /> : null}
            <Button variant="danger" label={confirmDelete ? copy('ยืนยันลบวัตถุดิบ', 'Confirm delete') : copy('ลบวัตถุดิบ', 'Delete ingredient')} onPress={remove} loading={saving} style={{ flex: 1 }} />
          </View>
        </Surface>
      ) : null}
    </AppScreen>
