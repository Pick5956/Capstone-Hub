...
export default function AddOrderItemScreen() {
  ...
  return (
    <AppScreen
      ...
      subtitle={menu ? `${money(menu.price, language)} · ${order?.table?.display_label || order?.order_number || ''}` : copy('กำลังโหลด', 'Loading')}
      topLevel={false}
      footer={!tabletWorkspace && menu ? <ActionDock label={copy(`${quantity} รายการ`, `${quantity} items`)} value={money(total, language)}><Button icon="add" label={copy('เพิ่มเข้าออเดอร์', 'Add to order')} onPress={add} loading={saving} disabled={missingRequired || !menu.is_available} /></ActionDock> : undefined}
    >
      ...
      {menu ? (
        <View style={{ flexDirection: tabletWorkspace ? 'row' : 'column', alignItems: 'flex-start', gap: spacing.lg }}>
          ...
          <Surface>
            ...
            {(menu.option_groups || []).filter((group) => group.is_active).map((group, groupIndex) => {
              ...
              const expanded = tabletWorkspace || expandedGroup === groupIndex;
              return <View key={group.ID} style={{ borderTopWidth: 1, borderTopColor: palette.border }}>
                <Pressable accessibilityRole={tabletWorkspace ? undefined : 'button'} accessibilityState={tabletWorkspace ? undefined : { expanded }} disabled={tabletWorkspace} onPress={() => setExpandedGroup((current) => current === groupIndex ? -1 : groupIndex)} style={({ pressed }) => ({ minHeight: 54, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, paddingVertical: spacing.sm, opacity: pressed ? 0.72 : 1 })}>
                  <View style={{ minWidth: 0, flex: 1 }}><Text style={typeScale.cardTitle}>{group.name}</Text><Text style={[typeScale.caption, { color: palette.muted }]}>{copy(`${minSelect > 0 ? 'ต้องเลือก' : 'เลือกได้'} ${minSelect}-${group.max_select}`, `${minSelect > 0 ? 'Required' : 'Optional'} ${minSelect}-${group.max_select}`)}</Text></View>
                  <Text style={[typeScale.caption, { color: palette.muted }]}>{selected}/{group.max_select}</Text>
                  {!tabletWorkspace ? <AppIcon color={palette.muted} name={expanded ? 'chevron-up' : 'chevron-down'} size={17} /> : null}
                </Pressable>
                {expanded ? options.map((option) => { const active = selectedOptionIds.includes(option.ID); return <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: active }} key={option.ID} onPress={() => toggle(ids, option.ID, group.max_select)} style={({ pressed }) => ({ minHeight: 50, flexDirection: 'row', alignItems: 'center', gap: spacing.md, borderTopWidth: 1, borderTopColor: palette.border, backgroundColor: pressed ? palette.surfaceSubtle : palette.surface, paddingVertical: spacing.sm, opacity: pressed ? 0.76 : 1 })}><Text style={{ flex: 1, color: palette.text, fontSize: 14, fontWeight: '600' }}>{option.name}</Text>{option.price_delta ? <Text style={{ color: palette.muted, fontSize: 13, fontWeight: '700' }}>+{money(option.price_delta, language)}</Text> : null}<AppIcon color={active ? palette.accent : palette.muted} name={active ? 'checkbox' : 'square-outline'} size={22} /></Pressable>; }) : null}
              </View>;
            })}
          ...
          <Surface>
            ...
            {tabletWorkspace ? <Button icon="add" label={copy('เพิ่มเข้าออเดอร์', 'Add to order')} onPress={add} loading={saving} disabled={missingRequired || !menu.is_available} /> : null}
          </Surface>
          </View>
        </View>
      ) : null}
